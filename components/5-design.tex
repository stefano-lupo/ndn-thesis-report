...
\begin{figure}[H]
    ...
    \caption{Taxonomy of data found in MOGs, with corresponding data in \game{} and the protocols used to synchronize the data in \game{}.}
...
Due to the variation in the data found in MOGs, multiple synchronization protocols are used, as shown in \ref{fig:des:taxonomy-with-data}. These are discussed in the following sections. 
...
As outlined in \refsec{sec:sota:dead-reckoning}, \textit{dead reckoning (DR)} is a necessity for a fast paced game such as \game{}. As such, the all updates to mobile game objects will contain the game objects velocity vector, as well as the updated position, allowing other non primary copy holders to extrapolate the position of game objects between remote updates.
